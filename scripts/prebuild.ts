...
const templateFiles = templateNames.map(filename => fs
  .readFileSync(path.join(envFilesDir, filename), { encoding: 'utf-8' })
);

const targetNames = templateNames.map(target => target.match(/^(.+)\.template$/)[1]);
